...
# same for europe data

# calculate MSI (from https://doi.org/10.1016/j.ecolind.2017.05.033)

MSI_MC_func_eu <- function(dataset,
                        nsim=1000, # number of Monte Carlo simulations
  ...
  dataset <- ddply(dataset, .(code_sp, Species), 
                   .fun = function(x){
                     x$Index2 <- (x$Index/x$Index[1])*100
                     x$Index_SE2 <- (x$Index_SE/x$Index[1])*100 # Taylor expansion
                     return(x)
                   })
  ...
  index <- dataset$Index2
  
  se <- dataset$Index_SE2
...
# Remove the buggy Beale index from NbClust
